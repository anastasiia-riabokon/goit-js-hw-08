...
ref.galleryList.addEventListener('click', event => {
  ...
  function openModalPhoto() {
    const markupOriginImg = `<div class = "modal"><img
      src="${href}"
      alt="${images.description}"
    /></div>`;
    const modal = basicLightbox.create(markupOriginImg, {
      onShow: () => {
        window.addEventListener('keydown', onModalPhotoClose);
      },
      onClose: () => {
        window.removeEventListener('keydown', onModalPhotoClose);
      },
    });

    modal.show();

    function onModalPhotoClose(event) {
